        ...
        while True:
            ...
            try:
                ...
                if feed_event:
                    ...
                    logging.getLogger('petfeedd').info("Before Weight: " + str(before_weight))
                    while after_weight - before_weight < 35:
                        self.feed(feed_event.size)
                        after_weight = weigh()
                    logging.getLogger('petfeedd').info("\nBefore Weight:" + str(before_weight) + "\nAfter weight:" + str(after_weight))
                    note = Notification()
                    note.text = self.config["general"]["name"] + " dispensed " + \
                        str(after_weight - before_weight) + "g of food at " + \
                        datetime.datetime.now().strftime("%I:%M%p on %B %d, %Y")

                    self.notification_queue.put(note)
                    feed_event.weight = after_weight
                    feed_event.save()
